...
# 話者ラベル。行頭の漢字・カタカナ2〜8字のあとに、`「〜」` 形か
# `）` に本文が続く形を話者と見る。ひらがなを含めると `次に「保存」を押す`
# のような普通の文を話者と誤認する。`「` 形は「その行が `」` で終わる」か
# 「その行に `」` が無い（発話が次行へ続く）」のどちらか。`画面「設定」を…`
# のように行中で引用が閉じて文が続くものは話者ではない。
# `）` は `手順）` のような節見出しと区別するため、後続の文字を要求する。
# 名には長音符（ー）と踊り字（々）を含める。含めないと `ユーザー「…` や
# `佐々木「…` のような規定外話者が話者と認識されず素通りする。
SPEAKER = re.compile(r"^([ァ-ヶー一-龯々]{2,8})(?:「(?:.*」\s*$|[^」]*$)|）.)")
