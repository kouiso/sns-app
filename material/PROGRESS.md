...
ブランチ `claude/sns-app-qa-gates-20260924` のPRで入れた。実装はDevin、レビューはClaudeである。
...
- 自己テストは26本になった。1本ずつ実行して全て通る。CIでも26本を1本ずつ実行する。
- 検査の対象は教材本文（`curriculum/`）だけである。本文がまだ無いので、今は全て「未判定」
  （終了コード3）で終わる。D1 §8-3 のとおり、0件を合格とは扱わない。
- 開始状態の混入検査は、開始状態の定義（G0の未決項目）が無いので、本文ができても未判定のままである。
- 捨て試作の章にも構造と方針同期を参考として掛けている。合否には数えない。
