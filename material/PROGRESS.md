...
| **B14 カスタム SMTP の要否** | **決着（2026-07-28。16 §B14）。**一次資料で確認したところ、通数ではなく**宛先**で詰まると判明した |
| B群の残り（P1 で決める20件） | Phase 1 の作業。今は着手しない。ほかに B13・B29・B30 が Phase 2 の前まで |

#### B14 は「通数」ではなく「宛先」の問題だった（2026-07-28 決着）

**当初の前提が間違っていた。**「Supabase 標準のメール送信は1時間に2通まで」を制約として
4つの文書に書いていたが、一次資料を読むと**それより先に効く制限**があった。

**標準の送信は、組織のメンバーとして登録したアドレスにしか届かない。**
演習用に適当なアドレスでアカウントを作っても、**1通目から届かない**。
つまり通数を待つ意味が無い。

決定（16 §B14）:

1. メール確認を教える章は**学習者本人のアドレス1つ**で行う（自分のプロジェクトの所有者なので届く）
2. 複数アカウントが要る章は**メール確認を切った状態**で進める。切る手順と、
   なぜ本番では切らないかを教材に書く
3. **カスタム SMTP は教材の必須手順にしない**（第三者サービスへの登録は挫折ポイント）

波及: B1 章分割の順序に制約が付いた（確認を教える章 → 確認を切る手順 → 複数アカウントの章）。
反映先は 01・02・03・16。
