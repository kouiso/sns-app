...
| 捨て試作を SDK 54 で作り直す | **一部完了（2026-07-28）。** `listings/` 2件を expo 54.0.36 / RN 0.81.5 へ。`snapshots/` は未反映（リポジトリに無い）。あわせて**章の作成コマンドと 16 の決定文が別テンプレートを指していた食い違いを訂正**（章＝blank-typescript、決定文＝default だった） |
