...
最終更新: 2026-09-24 / フェーズ: **Phase 1 詳細設計**（`14_マスターロードマップ.md`）

## 2026-09-24 QA自己テストの修正、G3の残り4検査、G4受領証の検査

この変更一式はPRで入れる。実装はDevin、レビューはClaudeである。

- 失敗していた既存QA 6本を、試作環境記録§3の処置案どおりに直した。
  `check_zip_reference` は、EPUB の参照先を見る検査と PDF のリンクを見る検査の2本に作り直した。
- G3の残り4検査（構造、開始状態の混入、方針同期、開発ログの存在確認）を実装した。
- G4受領証の検査（D15-8の7項目とD15-6のファミリー確認）を実装した。
- 自己テストは27本になった。1本ずつ実行して全て通る。CIでも27本を1本ずつ実行する。
- G3の文体チェックも、既定の対象を捨て試作の章から教材本文（`curriculum/`）に移した。
- 本文を読む検査（文体、構造、方針同期）の対象と、存在確認が要求する章は、どちらも教材本文（`curriculum/`）だけから取る。
  本文がまだ無いので、今は全て「未判定」
  （終了コード3）で終わる。D1 §8-3 のとおり、0件を合格とは扱わない。
- 開始状態の混入検査は、開始状態の定義（G0の未決項目）が無いので、本文ができても未判定のままである。
- 捨て試作の章にも文体、構造、方針同期を参考として掛けている。合否には数えない。
  構造の指摘が2件ある（`chapter-expo-first-screen.md:18` の段落が4文、
  `chapter-live-reload.md:71` の発話が5文）。捨て試作の本文は直していない。

| ゲート | 状態 | 根拠 |
|---|---|---|
| G3 章ごとの機械検査 | 未通過（章なし） | 5検査すべて実装済み。本文が0件なので未判定 |
| G4 別系統AIの査読 | 未通過（章なし） | 受領証は0件。検査は実装済み。B38が未決 |

G1とG2は09-23の表から変わっていない。09-23の「次の操作」3は、この節で済んだ。

## 2026-09-23 引き継ぎ（Codex→Claude）と再確認

WSLで止まったCodexセッションの作業を回収した。以後はClaudeが進める。
回収物は #12 で main に入った（`7f33280`）。回収直後の `62a9dc9` はファイル名を英語に直す前の版なので、参照しない。
09-19の判断と記録は変えていない。今回は現状を測り直しただけである。

### 再確認の結果（Mac mini）

- 章表の検査を再実行した。終了コード0、`draftStructureValid=true`。
- `declaredGateInputsComplete=false` のまま。31章、FR15件、エラー0件。
- 未解決の依存は84件、未記入の宣言は62件ある。
- 章表parserのテスト20本は全て通った。
- iPhone 12 miniはこのMac miniと接続済みである。
- 全アプリ一覧（`--include-all-apps`）にExpo Goは無い。導入はまだ終わっていない。

### ゲートの現状

G1とG2は執筆前のゲート、G3とG4は章ごとのゲートである。章の本文はまだ無い。

| ゲート | 状態 | 根拠 |
|---|---|---|
| G1 章分割の凍結 | 未通過 | 18はDRAFT / NON-G1。宣言の不足と局長承認が残る |
| G2 文体仕様の凍結 | 未通過 | 最終判定は未実施。B41、B42、ブラインド試験の扱い、局長承認が残る |
| G3 章ごとの機械検査 | 未通過（章なし） | 5検査のうち4件が未実装 |
| G4 別系統AIの査読 | 未通過（章なし） | 受領証は0件。受領証の検査も未実装。B38が未決 |

### 次の操作

1. iPhoneにExpo Goを入れ、Androidと同じ手順でiOS A0を測る。
2. iOS A0を独立レビューで確かめ、S1をPASSにする。
3. 失敗中の既存QA 6本を、試作環境記録§3の処置案どおりに直す。

## 2026-09-19: V3.5の実行 — 限定単位はPARTIAL / BLOCKED

基準は `7f04545`、開始時の作業ツリーに差分なし。実行判断と完了条件は
`decisions/20260919-chapter-table-draft-check.md` に保存した。Phase 0の承認を取り消していない。
...
iPhone 12 miniの画面取得経路も確立したが、Expo Go導入待ちで表示試験は未実行。

### 完了した範囲

- S0: 04§10、05§4、06§7、08§12の固定入力109件を16のIDへ対応付けた。
  DB13の元項目を保持し、重複を統合。新たにB70〜B122を採番したが、判断済みとはしていない。
  対応証拠は `decisions/20260919-unresolved-input-handling.md`、未決の正本は16のまま。
- S2の下書き: 18をA0〜E全体・31章の**構成仮説**として作成。FR1〜15は一次実装章へ対応する。
  両OSの物理A0証拠がまだそろわないため、章数・順序・境界の実測による確定は未完了。
...
- 共通parserと正例・負例のテストを追加。実表は `draftStructureValid=true`、
  ...
  20テスト、変更2ファイルのruffとmypyを通過。mypyはimport先の既存ファイルを報告対象から
  分けた検証であり、既存 `markdown_scan.py:123` の型エラーを修正・合格したとはしていない。
- 既存QAを17本個別に再実行し **11 PASS / 6 FAIL**。6件ごとのB31処置案と保持すべき検出能力を
  `decisions/20260919-prototype-env-and-qa-check.md` に記録した。修正・G0全体の合格は未完了。
...
- 最終の独立コードレビューで今回の12ファイルの差分はAPPROVE。
  途中で見つかった期限判定、章履歴・例外範囲の偽PASS、型エラー、案内の同期漏れを修正し再検証した。
  これは今回のローカル差分へのレビューであり、教材のG4受領証や局長の正式承認ではない。
...
| 対象 | 状態・不足 | 担当 | 次の操作 |
|---|---|---|---|
| 物理Android A0 | PASS（USB転送での初期表示・編集反映限定）。QR/Wi-Fiは未検証 | 実機検証担当 | iOSを独立確認し、初心者向け開始手順と補助経路を区別して章案へ反映 |
| 物理iOS A0 | 実機・画面取得経路あり。Expo Go未導入で未実行 | 実機検証担当 | App Storeの入手・インストール操作は実施済み。端末のApple Account認証後に到達性・初回表示・編集反映を独立検証 |
...
| 章表の確定 | 未完了。A0/D4/Authの実測、DB13等、C8の節目指定が残る | 設計担当＋局長 | 証拠と判断を章境界へ反映し、P1/G1の全条件と必要承認を別途判定 |

**Androidは限定PASSだがiOSのPASSがないため、限定単位のDONEを宣言しない。**
端末の場所は回答済み。iPhoneへのExpo Go導入を依頼し、Android実証を先行した。
...
- **章分割表（`material/18-chapter-split-table.md`）が存在しない。**D1-6 が正本と定め9か所が参照しているが実体が無い。
...
| 捨て試作を SDK 54 で作り直す | **一部完了（2026-07-28）。** `listings/` 2件を expo 54.0.35 / RN 0.81.5 へ。`snapshots/` は未反映（リポジトリに無い）。あわせて**章の作成コマンドと 16 の決定文が別テンプレートを指していた食い違いを訂正**（章＝blank-typescript、決定文＝default だった） |
| G6 実走検証（`exec`） | **未完走**。共有フックが `npm install` を拒否するため。詳細は `prototype-chapter/道具検証の記録.md` §12 |
| 章2への G4 別ファミリーレビュー | 未実施（1章目のみ実施済み） |
| **B14 カスタム SMTP の要否** | **期限を過ぎている。** 16 は「捨て試作で実測」と決めていたが、捨て試作は完了したのに測っていない。**依然として未決**。下の「B14 の期限切れ」と「B14 の一次資料調査」を参照 |
...
#### B14 の一次資料調査（2026-07-28・未決のまま）

上記の実測に着手する前に、Supabase 公式ドキュメントを読んだ。**通数（1時間2通）より
先に効く可能性のある制限**が見つかったが、**自分のプロジェクトでは未確認**であり、
B14 はこの調査だけでは閉じない。

一次資料が示す内容（未実測）:

- 標準の送信は、組織のメンバーとして登録したアドレスにしか届かない可能性がある。
  演習用に適当なアドレスでアカウントを作った場合、1通目から届かないかもしれない
- メール確認は認証プロバイダの設定画面で無効化できる可能性がある

**次にやること**: 宛先制限が実際に効くかの実機確認、通数の実測（N1〜N3。08 §10.3）、
メール確認の無効化操作の実機確認（08 §10.4 C3）の3点。いずれも未着手。
完了して結果を記録するまで B14 を決着扱いにしない。

詳細と出典は `16_決定バックログ.md` §B14 調査メモ。反映先は 01・02・03（一次資料調査の
存在のみ追記済み。決定はまだしていない）。
