...
| プロフィール編集（#7） | **上げた画像の URL を `users` に書き戻す** | `users`（`avatar_url` / `header_url`） | update | `users_update_own` | **U21**（下記。書き戻すか、パスから導くか） |
...
**★ 画像を上げただけでは画面に反映されない（U21。2026-08-09 追記。codex 指摘）。**
05 の `users` は `avatar_url` と `header_url` を列として持ち、プロフィール画面（#6）は
**`users` を読んで表示する**。ところが 2026-08-09 の初版は Storage へ書く行しか置いておらず、
**`users` を更新する行が抜けていた。**そのままだと、画像を上げても画面は
古い URL（初回は NULL）を返し続ける。**08 §2 の FR2「画像を上げて表示に反映される」が
原理的に満たせない状態**だった。上の表に行を足したが、**やり方が2つあってまだ決めていない。**

| 案 | 中身 | 代償 |
|---|---|---|
| (a) URL を `users` に書き戻す | 上げたあと `users_update_own` で `avatar_url` を更新する | **書き込みが2段になる。**Storage は成功したが `users` の更新に失敗すると、実体はあるのに画面に出ない状態が残る（§2.4 の「投稿の途中失敗」と同型） |
| (b) パスから導く | `users` に URL を持たず、`<利用者のid>/avatar.<拡張子>` から画面側で組み立てる | **05 の列が要らなくなる**（05 の修正）。拡張子が可変なので、拡張子を固定するか別に持つ必要がある |

(a) を採るなら **§2.3 の `users_update_own` が `avatar_url` を許すこと**が前提になり、
U19（`users` の update を編集してよい列だけに絞る）の対象列にこの2つを含める必要がある。
**U19 と U21 は同じ席で決める。**
どちらの案でも **B66**（アバターのパスが固定名なので、差し替えても URL が変わらず
古い画像が返るかもしれない件）は別に残る。
...
**⚠️ このポリシーも列を制限していない（U19。2026-08-09 追記）。**
行さえ自分のものなら、**`created_at` と `updated_at` を書き換えられる**。
05 はこの2列に既定値を持たせており、**利用者が入れる値ではない**。
`updated_at` を偽れば「いつ直したか」の記録が意味を失う。
`notifications`（U4）・`posts`（U19）と同じ「RLS では列を絞れない」型で、
**本書はこの3か所を同じ席で決める**（列単位の `GRANT UPDATE(...)` か、専用の関数か）。
なお `users.updated_at` を**誰がいつ更新するか**そのものが未決である（16 B64）。
更新の主体が決まらないと、どの列を許すかも決まらない。
...
| `posts_insert_own` が親の生死を見ないこと | **論理削除された投稿にリプライ・リポストを作れる**（U20。下記） |
...
**⚠️ `posts_insert_own` は `author_id` しか見ておらず、`reply_to_post_id` /
`repost_of_post_id` の指す親が生きているかを見ていない（U20。2026-08-09 追記）。**
削除される前に id を知っていれば、**削除済み投稿へのリプライやリポストを後から作れる。**
外部キーは親の行が存在することしか確かめず、`deleted_at is null` を要求しない。
できた子は `posts_select` を通るので**読める側に出る**が、**親は誰にも select できない。**
つまり「親のいないリプライ」が一覧に並ぶ。さらに §3.2 のリプライ・リポストの通知トリガーが動き、
**受け取った人が開けない投稿を指す通知が届く。**
`likes` / `bookmarks`（§2.6）と同じ穴で、**U20 はこの2つも含めて1件として扱う。**
塞ぐなら `with check` に親の `deleted_at is null` を足すが、
**U2（親を参照する `exists` が `posts` の RLS を通るか）の実測が先である。**
...
| U19 | **`posts` と `users` の update を、編集してよい列だけに絞るか**（列単位の `GRANT UPDATE(deleted_at)` か、論理削除専用の `security definer` 関数か） | §2.3・§2.4。現状の `posts_update_own` / `users_update_own` はどちらも行の持ち主しか見ておらず、`posts` は **`created_at`・`body`・`reply_to_post_id`・`repost_of_post_id`**、`users` は **`created_at`・`updated_at`** を後から書き換えられる。FR3 に投稿の編集は無いのに編集を許しており、「削除は update で行う契約」という §2.4 自身の説明とも食い違う。`created_at` の書き換えでタイムラインの並びを動かせ、`reply_to_post_id` の書き換えで既存のスレッド構造を作り替えられる。**U4（`notifications` の列制限）と同じ2案で、3か所とも同じ席で決めるのが素直**（`users.updated_at` を誰が更新するかは 16 B64） | パートB の投稿の章の実装PR前（U4 と同じ席） |
| U20 | **論理削除された投稿に いいね／ブックマーク／リプライ／リポスト を作れてよいか** | §2.4・§2.6。`likes_insert_own` / `bookmarks_insert_own` は「誰として書くか」しか見ておらず、**親の投稿が生きているかを見ていない**。**`posts_insert_own` も同じで、削除済み投稿へのリプライ・リポストを作れる**（親を select できないのに子は読める＝親のいないリプライが一覧に並ぶ）。外部キーは親の存在しか確かめないので、削除前に id を知っていれば削除後でも作れる。作れると §3.2 のトリガーが動き、**受け取った人が開けない投稿を指す通知が届く**。**B27 を「DB 側で隠す」に決めてもこの経路は閉じない**（B27 と 08 §5.6 の追加は子テーブルの select の話で、insert には触れていない）。塞ぐなら `with check` に親の `deleted_at is null` を足すが、**U2（親を参照する `exists` が `posts` の RLS を通るか）の実測が先** | 章分割表の凍結前（B1）。B27 と同じ席で決める |
| U21 | **上げた画像の URL を `users` に書き戻すか、Storage のパスから導くか** | §1.2。05 の `users` は `avatar_url` / `header_url` を持ち、プロフィール画面はそこを読む。**Storage へ書くだけでは画面に反映されない**（08 §2 の FR2 が満たせない）。(a) 書き戻すなら書き込みが2段になり途中失敗の扱いが要る。(b) パスから導くなら **05 の列が不要になる**（05 の修正）。**U19 の対象列に `avatar_url` / `header_url` を含めるかがこれに従属する** | パートB の画像の章の実装PR前（U12・U19 と同じ席） |
