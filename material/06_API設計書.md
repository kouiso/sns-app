...
`02_技術選定書.md` §6 のとおり、次は前提ごと消滅している。復活させない。

- 自前 API のエンドポイント一覧 / `openapi.yaml` を正本に置く方式 / `@nestjs/swagger` との差分照合
- Authorization ヘッダーとリフレッシュトークンのローテーション手順
- 自前 WebSocket のイベント発行契約
...
「効くポリシー」は §2 で定義するポリシー名。Auth と Storage は DB のポリシーではないので
その旨を書く。「依存する未決定」が埋まっている行は、**その未決定が片づくまで
実装の章を書けない**。
...
| 投稿作成（#4） | 画像を上げる | Storage `post-media` バケット | insert | `post_media_objects_write_own`（§5） | B21（MIME・容量をサーバー側で止めるか） |
| 投稿作成（#4） | 投稿を作る | `posts` | insert | `posts_insert_own` | B19（既定値を誰が入れるか）、05 §4 課題3・課題4 |
| 投稿作成（#4） | 画像の行を作る | `post_media` | insert | `post_media_write_own_post` | 05 §4 課題2（最大4枚をDB層でどう保証するか） |
| 投稿作成（#4） | 本文からタグを取り出して結ぶ | `hashtags` / `post_hashtags` | insert | `hashtags_insert_authenticated`, `post_hashtags_write_own_post` | §7 U9（タグ本体の正規化） |
...
| プロフィール編集（#7） | アバター・ヘッダーを上げる | Storage `avatars` バケット | insert / update | `avatars_objects_write_own`（§5） | B21 |
| フォロー中／フォロワー一覧（#8） | 一覧を取る | `follows` / `users` | select | `follows_select`, `users_select` | B25、§7 U1（1画面か2画面か） |
...
（更新0行になる）ところを見る。**「エラーが出ない」ことの意味は §6.3 で扱う。**
...
`with check` は「1行が満たすべき条件」しか書けないので、**枚数のような行をまたぐ条件は
`with check` では書けない**。CHECK 制約・トリガー・アプリ層検証のどれを採るかが決まるまで、
この節は4枚制限を持たない。
...
`bookmarks` は他人の行を select して**0件が返る**ことを見る（§6.3）。
...
加えて、**タグ本体の正規化（大文字小文字・全角半角・前後の空白）を誰がやるかが決まっていない**（§7 U9）。
`tag` は UNIQUE なので、`Expo` と `expo` を別行にするかどうかがそのまま検索結果の分断になる。
...
購読は次の5項目を1組で決める。**1つでも欠けると、画面が黙って古いままになる。**
...
| 画面 | 購読するもの | フィルタ | 画面への反映 | 解除の時点 |
|---|---|---|---|---|
| タイムライン（#3） | `posts` の insert | **フォロー中の集合で絞れるかは未確定**（§7 U6） | 先頭に足す。既に並んでいる id は足さない | 画面を離れるとき／アプリが背面へ回るとき |
| 通知（#9） | `notifications` の insert | `recipient_id` が自分 | 一覧の先頭に足し、未読の数を増やす | 同上 |
| 投稿詳細（#5） | `likes` の insert / delete | `post_id` がこの投稿 | いいね数を増減する | 画面を離れるとき |
| 投稿詳細（#5） | `posts` の insert（リプライ） | `reply_to_post_id` がこの投稿 | スレッドの末尾に足す | 同上 |
...
1. **購読が RLS を通るか。**通るなら、`notifications` は `recipient_id = auth.uid()` の行しか
   配信されないので、フィルタは二重の守りになる。通らないなら、**フィルタが唯一の守りになり、
   フィルタを書き忘れた時点で他人宛の通知が端末へ届く。**どちらであるかで危険度が変わる（§7 U6）。
...
`post-media` も同じ形で、`bucket_id` を差し替える。
...
Supabase から返るものを6つに分ける。**分けるのは、画面での出し方が違うからである。**

| # | 分類 | 具体例 | どこで起きるか |
|---|---|---|---|
| E1 | 入力の誤り（DB の制約が拒否した） | `username` の UNIQUE 違反、文字数超過、CHECK 違反 | #13-2 / #7 / #4 |
| E2 | 認可の拒否（RLS が拒否した） | 他人の行への insert / update | 書き込みのある全画面 |
| E3 | 認証の状態 | 未ログイン、メール未確認、セッション切れ | 全画面 |
| E4 | 通信の失敗 | オフライン、タイムアウト、Realtime の切断 | 全画面 |
| E5 | Supabase 側の制限 | メール送信の上限、ログインのレート制限 | #2 / #13 / #14 |
| E6 | 想定外 | 上のどれでもないもの | — |

### 6.2 画面への出し方の原則

**本書は分類までを決め、文言と表示位置は `04_詳細設計書.md`（目次1「エラー時の表示仕様」）で決める。**
本書が決めるのは次の3つだけである。

1. **E1 は入力に紐づけて出す。**どの入力欄が悪いのかが分かる形にする。
   `username` の重複は **DB の UNIQUE 制約違反がそのままアプリへ返る**（05 users 節）ので、
   これを「このIDは使われています」に変換する対応表が要る。
2. **E2 は利用者の操作の誤りではない。**正しく作ったアプリでは E2 は起きない。
   E2 が画面に出た時点で、**それは実装の欠陥である**。教材では E2 を
   「利用者に見せるもの」ではなく「開発中に自分が見るもの」として扱う。
...
エラーの扱いは1章にまとめない。**E1 は #13-2（表示IDの重複）で、E2 は RLS の章で、
E5 はメール確認の章で、それぞれ目の前の機能が動かない文脈が生まれた瞬間に扱う**
（`11_ターゲット・ペルソナ・UX定義.md` 原則6「重いトピックは必要になった瞬間に最小限」）。
E4（通信の失敗）は `08_テスト計画書.md` 目次7（オフライン・再接続）と対になる。
...
| B番号 | 本書のどこが止まるか |
|---|---|
| B4 | §1.2 タイムラインの行。プッシュ型ならテーブルが1つ増え、§2 にポリシーが1組増える |
| B7 | §3.2 の表に5行目が要るか |
| B19 | §3.1（トリガーはクライアントではない）、§1.2 の insert 行 |
| B21 | §5.3 の2 |
| B22 | §4.3 の3項目 |
| B24 / B25 | §1.2 の #6 / #8 / #10 / #13-2 の行 |
| B27 | §2.4 の `using (deleted_at is null)` |
| B28 | §3.2 全体 |
| 05 §4 課題2 | §2.5（枚数は `with check` では書けない） |
| 05 §4 課題3・課題4 | §1.2 の #4 / #5 の insert 行 |
| 05 §4 課題5 | §1.2 の検索（#10）の行。**この行だけ書けていない** |
| 05 §4 課題6 | §3.2 の `type` |
...
| U3 | 05 §5 の RLS 表の `notifications` 行の文言（「UPDATE は既読フラグのみ本人可」→「行は本人のみ。列の制限は別手段」） | 表のまま SQL に書き下ろすと、教材のポリシーが嘘になる。**05 の修正であって本書では直せない** | 05 の局長承認前 |
| U4 | 既読化を列単位 GRANT で塞ぐか、専用関数で塞ぐか（どちらでも `revoke update` は要る） | §2.7 の SQL が2案のまま。教材の章に2案は載せられない | パートB の通知の章の実装PR前 |
| U5 | `notifications` の delete を誰かに許すか（通知の削除は機能要件に無い） | §2.7 でポリシーを書かない扱いにしたが、根拠が「要件に無い」だけである | パートB の通知の章の実装PR前 |
| U6 | Realtime の購読が RLS を通るか。フィルタに集合を使う条件を書けるか | §4.2 のフィルタ列が埋まらない。通らない場合、フィルタの書き忘れが即座に他人の通知の漏れになる。**実測が要る** | 本書（06）の凍結前 |
| U7 | 読み取りを `anon`（未ログイン）まで広げるか | §2 の全ポリシーの `to` 句。パートD で未ログイン閲覧を見せるかに直結する | 章分割表の凍結前（B1） |
| U8 | `post_media.order` の扱い（列名を変えるか、引用が要る旨を注記するか） | **SQL の予約語なので、教材のコード片をそのまま写経させると落ちる。**05 に注記が無い | パートB の投稿の章の実装PR前 |
...
| U13 | Storage の孤児ファイル（投稿の作成に失敗した画像、論理削除した投稿の画像）を片づけるか | §5.3 の3。片づけるなら削除の経路が要り、章が1つ増えうる | 章分割表の凍結前（B1） |
| U14 | RLS で弾かれた update / delete が「エラー」ではなく「0行」で返ることの実測 | §6.3 は本書で最も教材効果の高い1点だが、**測っていない。**測る前に断定して書けない | 本書（06）の凍結前 |
