...
#     2. 構造チェック（コードブロック25行以内・段落3文以内）  未実装
#     3. 開始状態の混入検査                                  未実装
#     4. 方針同期チェック（配布形式・不採用スタック名・人物名） 未実装
#     5. 開発ログの存在確認                                  未実装
#   2〜5 は Phase 2 のゲート実装で作る。それまで、このスクリプトが 0 で終わっても
...
"$TEXTLINT" "${TARGETS[@]}"
echo "文体チェック PASS（G3 の残り4検査は未実装。これは G3 通過ではない）"
