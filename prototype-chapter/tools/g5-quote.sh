...
# ★ 免除が使われたら、理由が書いてあっても**落とす**。
#   理由さえ書けばコードを載せた章でも照合を外せる状態は、
#   「指示を消して免除簿へ名前を足す」だけで緑にできることを意味する。
#   いま免除している章は0件なので、落としても誰も困らん。
#   本当に読み物だけの章が出てきた時に、人の承認を挟む形で改めて設計する。
if [[ ${#EXEMPTED[@]} -gt 0 ]]; then
  echo "G5 FAIL: 免除簿で照合を外した章がある: ${EXEMPTED[*]}" >&2
  echo "  この章のコードは誰も照合していない。いまは免除を認めていない" >&2
  echo "  読み物だけの章が本当に要るなら、人の承認を挟む形で設計し直すこと" >&2
  exit 2
fi
