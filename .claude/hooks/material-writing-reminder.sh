...
# PreToolUse hook: curriculum/**/*.md への Write/Edit 時に material-writing の6手順をモデルの文脈へ注入する。
#
...
# worktree（.../sns-app/.claude/worktrees/<name>）でも効くよう部分一致にする（過去バグ(3)）。
...
# curriculum 配下の .md のみ対象（sns-app の material/ は設計文書なので除く）。
# 絶対パス（*/curriculum/*）と、先頭がそのまま curriculum/ で始まる相対パスの両方を受ける。
# 実測(2026-07-15, v2.1.210)では Claude Code は常に絶対パスへ正規化して渡すが、
# それは観測された挙動であって文書化された契約ではない（Geminiレビュー指摘 PR#284 の横展開）。
[[ ( "$FILE_PATH" == */curriculum/* || "$FILE_PATH" == curriculum/* ) && "$FILE_PATH" == *.md ]] || exit 0
